...
Para determinar se o incremento de $4,00\%$ na taxa de conversão a favor do botão Verde possui significância estatística ou se decorre meramente de flutuações aleatórias do processo de amostragem, aplicou-se o teste Z de igualdade de duas proporções assintótico \cite{montgomery2018applied}. O teste foi configurado de forma bicaudal, sem a correção de continuidade de Yates. As hipóteses estatísticas formalizadas são:
...
De acordo com o referencial teórico assintótico, a variância amostral da diferença entre duas proporções independentes sob a hipótese nula é governada pelo erro padrão teórico ($\sigma_{\text{teórico}}$), calculado pela Equação~\ref{eq:sigma_teorico}.

\begin{equation}
	\sigma_{\text{teórico}} = \sqrt{P(H0) \cdot (1 - P(H0)) \cdot \left( \frac{1}{n_A} + \frac{1}{n_B} \right)}
	\label{eq:sigma_teorico}
\end{equation}
...
\begin{lstlisting}[language=R, caption={Teste A/B com teste Z de proporções e simulação de Monte Carlo.}, label={lst:teste_ab}]
library(ggplot2)
library(dplyr)
set.seed(42)
